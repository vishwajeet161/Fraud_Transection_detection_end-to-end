...
class Raw_Data_Validation:
    ...
    def fetch_values_from_schema(self):
        try:
            ...
            file = open('Prediction_Logs/valuesfromschema_Validation_Log.txt','a+')
            ...
            file = open('Prediction_Logs/valuesfromschema_Validation_Log.txt', 'a+')
            self.logger.log(file,'Value Error : Value not Found inside schema_Prediction.json')
            file.close()
            raise ValueError
    ...
    def createDirectoryFor_GoodBadRawData_MainFile(self):
        try:
            path = os.path.join("Prediction_Raw_Validated_File/", "Good_Raw_MainFile/")
            if not os.path.isdir(path):
                os.makedirs(path)

            path = os.path.join("Prediction_Raw_Validated_File/", "Bad_Raw_MainFile/")
            if not os.path.isdir(path):
                os.makedirs(path)
    ...
    def createDirectoryFor_GoodBadRawData_AdditionalFile(self):
        try:
            path = os.path.join("Prediction_Raw_Validated_File/","Good_Raw_AdditionalFile")
            if not os.path.isdir(path):
                os.makedirs(path)

            path = os.path.join("Prediction_Raw_Validated_File/", "Bad_Raw_AdditionalFile")
            if not os.path.isdir(path):
                os.makedirs(path)
    ...
    def deleteExistingGoodDataTrainingDir_MainFile(self):
        try:
            path = "Prediction_Raw_Validated_File/"
    ...
    def moveBadFilesToArchiveBad_MainFile(self):
        ...
        try:
            source = 'Prediction_Raw_Validated_File/Bad_Raw_MainFile/'
            if os.path.isdir(source):
                path = 'PredictionArchiveBadData_MainFile'
                if not os.path.isdir(path):
                    os.makedirs(path)
                destination = 'PredictionArchiveBadData_MainFile/Bad_Data_' + str(date)+"_"+str(time)
    ...
    def moveBadFilesToArchiveBad_AdditionalFile(self):
        ...
        time = now.strftime("%H%M%S")
        try:
            source = 'Prediction_Raw_Validated_File/Bad_Raw_AdditionalFile/'
            if os.path.isdir(source):
                path = 'PredictionArchiveBadData_AdditionalFile'
                if not os.path.isdir(path):
                    os.makedirs(path)
                destination = 'PredictionArchiveBadData_AdditionalFile/Bad_Data_' + str(date)+"_"+str(time)
                ...
                if (re.match(mainfile_Regex,filename)):
                    ...
                    if len(split[2]) == main_lengthofdatestampinfile:
                        if len(split[3]) == main_lengthoftimestampinfile:
                            shutil.copy("Prediction_Batch_Files/Main_File/" + filename,"Prediction_Raw_Validated_File/Good_Raw_MainFile")
                            self.logger.log(file,'Valid File Name !! File moved to GoodRaw_Main Directory ::%s' % filename)
                        else:
                            shutil.copy("Prediction_Batch_Files/Main_File/" + filename,"Prediction_Raw_Validated_File/Bad_Raw_MainFile")
                ...
                if (re.match(additionalfile_Regex,filename)):
                    ...
                    if len(split[3]) == additionalfile_lengthofdatestampinfile:
                        if len(split[4]) == additionalfile_lengthoftimestampinfile:
                            shutil.copy("Prediction_Batch_Files/Additional_File/" + filename,"Prediction_Raw_Validated_File/Good_Raw_AdditionalFile")
                            self.logger.log(file,'Valid File Name !! File moved to GoodRaw_Additional Directory ::%s' % filename)
                        else:
                            shutil.copy("Prediction_Batch_Files/Additional_File/" + filename,"Prediction_Raw_Validated_File/Bad_Raw_AdditionalFile")
    ...
    def validate_NoOfCol_MainFile(self,noofcol_mainfile):
        try:
            ...
            for file in os.listdir('Prediction_Raw_Validated_File/Good_Raw_MainFile/'):
                csv = pd.read_csv('Prediction_Raw_Validated_File/Good_Raw_MainFile/' + file)
                if csv.shape[1] == noofcol_mainfile:
                    pass
                else:
                    shutil.move('Prediction_Raw_Validated_File/Good_Raw_MainFile' + file,'Prediction_Raw_Validated_File/Bad_Raw_MainFile')
    ...
    def validate_NoOfCol_AdditionalFile(self,noofcol_additionalfile):
        try:
            f = open("Prediction_Logs/columnValidationLog.txt", 'a+')
            for file in os.listdir('Prediction_Raw_Validated_File/Good_Raw_AdditionalFile/'):
                csv = pd.read_csv('Prediction_Raw_Validated_File/Good_Raw_AdditionalFile/' + file)
                if csv.shape[1] == noofcol_additionalfile:
                    pass
                else:
                    shutil.move('Prediction_Raw_Validated_File/Good_Raw_AdditionalFile' + file,'Prediction_Raw_Validated_File/Bad_Raw_AdditionalFile')
